...
module.exports = function(config) {
  ...
  router.get('/', async (req, res) => {
    ...
    if (config.printers?.crx_pro) {
      try {
        ...
        if (!printerRes.ok || !jobRes.ok) {
          throw new Error(`OctoPrint HTTP ${printerRes.status}/${jobRes.status}`);
        }
    ...
    if (config.printers?.k2) {
      ...
      results.k2 = { online: false, error: 'Not configured' };
    }
